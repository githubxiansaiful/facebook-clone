...
// ==========================================
// 🎨 SIDEBAR CUSTOM ICONS (MATCHING SCREENSHOT)
// ==========================================

// Meta AI Icon (Purple/Violet Swirl)
export function MetaAiIcon({ size = 36 }: { size?: number }) {
  return (
    <div
      style={{ width: size, height: size }}
      className="rounded-full flex items-center justify-center bg-gradient-to-tr from-purple-600 via-indigo-500 to-pink-500 text-white shadow-sm shrink-0"
    >
      <svg viewBox="0 0 24 24" width={size * 0.65} height={size * 0.65} fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
        <circle cx="12" cy="12" r="9" />
        <path d="M12 3a9 9 0 0 1 9 9 9 9 0 0 1-9 9" strokeDasharray="4 2" />
...
// Dashboard Icon (Multi-color bar chart)
export function DashboardIcon({ size = 36 }: { size?: number }) {
  return (
    <div
      style={{ width: size, height: size }}
      className="rounded-xl flex items-center justify-center bg-gradient-to-br from-blue-500/10 to-indigo-500/10 shrink-0"
    >
      <svg viewBox="0 0 36 36" width={size} height={size} fill="none">
        <rect x="6" y="14" width="6" height="16" rx="2" fill="#0084FF" />
        <rect x="15" y="8" width="6" height="22" rx="2" fill="#A033FF" />
        <rect x="24" y="18" width="6" height="12" rx="2" fill="#00D2D2" />
      </svg>
    </div>
  );
}

// Ad Center Icon (Megaphone)
export function AdCenterIcon({ size = 36 }: { size?: number }) {
  return (
    <div
      style={{ width: size, height: size }}
    ...
    >
      <svg viewBox="0 0 36 36" width={size} height={size} fill="none">
        <path d="M7 14h6l12-7v22l-12-7H7a2 2 0 0 1-2-2v-4a2 2 0 0 1 2-2z" fill="#0084FF" />
        <path d="M25 12c3 2 4 4 4 6s-1 4-4 6" stroke="#00C6FF" strokeWidth="2.5" strokeLinecap="round" />
        <rect x="10" y="24" width="4" height="6" rx="1.5" fill="#0052CC" />
      </svg>
    </div>
  );
}

// Ads Manager Icon (Blue Chart)
export function AdsManagerIcon({ size = 36 }: { size?: number }) {
  return (
    <div
      style={{ width: size, height: size }}
      className="rounded-xl flex items-center justify-center shrink-0"
    >
      <svg viewBox="0 0 36 36" width={size} height={size} fill="none">
        <rect x="7" y="18" width="5" height="11" rx="1.5" fill="#0084FF" />
        <rect x="15.5" y="12" width="5" height="17" rx="1.5" fill="#0084FF" />
        <rect x="24" y="6" width="5" height="23" rx="1.5" fill="#0084FF" />
      </svg>
    </div>
  );
}

// Birthdays Icon (Blue gift box with pink ribbon)
export function BirthdaysIcon({ size = 36 }: { size?: number }) {
  return (
    <div
      style={{ width: size, height: size }}
      className="rounded-xl flex items-center justify-center shrink-0"
    >
      <svg viewBox="0 0 36 36" width={size} height={size} fill="none">
        <rect x="6" y="15" width="24" height="15" rx="3" fill="#00A3FF" />
        <rect x="4" y="11" width="28" height="6" rx="2" fill="#0084FF" />
        <rect x="16" y="11" width="4" height="19" fill="#FF4B72" />
        <path d="M12 7c-2 0-3 2-2 4h4c1-2 0-4-2-4zm12 0c2 0 3 2 2 4h-4c-1-2 0-4 2-4z" fill="#FF4B72" />
      </svg>
    </div>
  );
}

// Events Icon (Calendar with Red top and star)
export function EventsIcon({ size = 36 }: { size?: number }) {
  return (
    <div
      style={{ width: size, height: size }}
      className="rounded-xl flex items-center justify-center shrink-0"
    >
      <svg viewBox="0 0 36 36" width={size} height={size} fill="none">
        <rect x="5" y="8" width="26" height="23" rx="4" fill="#FFFFFF" />
        <rect x="5" y="8" width="26" height="23" rx="4" stroke="#CED0D4" strokeWidth="1" />
        <path d="M5 12c0-2.2 1.8-4 4-4h18c2.2 0 4 1.8 4 4v4H5v-4z" fill="#FA3E3E" />
        <path d="M18 19l1.8 3.6 4 .6-2.9 2.8.7 4-3.6-1.9-3.6 1.9.7-4-2.9-2.8 4-.6z" fill="#FA3E3E" />
      </svg>
    </div>
  );
}

// Gaming Video Icon (Facebook Gaming Logo)
export function GamingVideoIcon({ size = 36 }: { size?: number }) {
  return (
    <div
      style={{ width: size, height: size }}
      className="rounded-xl flex items-center justify-center bg-[#0084FF] text-white shadow-sm shrink-0"
    >
      <svg viewBox="0 0 24 24" width={size * 0.65} height={size * 0.65} fill="currentColor">
        <path d="M4 4h16v16H4V4zm4 4v8h4v-3h4v3h4V8H8zm4 3h4v2h-4v-2z" />
      </svg>
    </div>
  );
}

// Messenger Kids Icon (Green/Yellow Bubble)
export function MessengerKidsIcon({ size = 36 }: { size?: number }) {
  return (
    <div
      style={{ width: size, height: size }}
      className="rounded-full flex items-center justify-center bg-gradient-to-tr from-[#31A24C] via-[#94D82D] to-[#F59F00] text-white shadow-sm shrink-0"
    >
      <svg viewBox="0 0 24 24" width={size * 0.65} height={size * 0.65} fill="currentColor">
        <path d="M12 2C6.48 2 2 6.13 2 11.23c0 2.91 1.45 5.51 3.73 7.15V22l3.47-1.91c.88.24 1.81.38 2.8.38 5.52 0 10-4.13 10-9.24S17.52 2 12 2zm1.09 12.44l-2.73-2.91-5.33 2.91 5.86-6.22 2.8 2.91 5.26-2.91-5.86 6.22z" />
      </svg>
    </div>
  );
}
...
export function MetaBusinessSuiteIcon({ size = 36 }: { size?: number }) {
  return (
    <div
      style={{ width: size, height: size }}
      className="rounded-full flex items-center justify-center bg-gradient-to-br from-blue-600 to-indigo-700 text-white shadow-sm shrink-0"
    >
      <svg viewBox="0 0 24 24" width={size * 0.65} height={size * 0.65} fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
        <circle cx="12" cy="12" r="9" />
...
// Orders and Payments Icon (Credit Card)
export function OrdersPaymentsIcon({ size = 36 }: { size?: number }) {
  return (
    <div
      style={{ width: size, height: size }}
      className="rounded-xl flex items-center justify-center bg-gradient-to-tr from-slate-900 via-blue-950 to-indigo-900 text-white shadow-sm shrink-0"
    >
      <svg viewBox="0 0 24 24" width={size * 0.65} height={size * 0.65} fill="none" stroke="currentColor" strokeWidth="2">
        <rect x="2" y="5" width="20" height="14" rx="3" fill="currentColor" fillOpacity="0.2" />
...
// Pages Icon (Orange flag)
export function PagesIcon({ size = 36 }: { size?: number }) {
  return (
    <div
      style={{ width: size, height: size }}
      className="rounded-xl flex items-center justify-center shrink-0"
    >
      <svg viewBox="0 0 36 36" width={size} height={size} fill="none">
        <path d="M9 6v24M9 6h17l-4 7 4 7H9" fill="#FF7A00" stroke="#FF7A00" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </div>
  );
}

// Play Games Icon (Game controller)
export function PlayGamesIcon({ size = 36 }: { size?: number }) {
  return (
    <div
      style={{ width: size, height: size }}
      className="rounded-xl flex items-center justify-center bg-[#0084FF] text-white shadow-sm shrink-0"
    >
      <svg viewBox="0 0 24 24" width={size * 0.65} height={size * 0.65} fill="currentColor">
        <path d="M21.58 7.19C21.19 6.47 20.35 6 19.46 6H4.54c-.89 0-1.73.47-2.12 1.19-.39.73-.39 1.62 0 2.35l2.25 4.5A2.5 2.5 0 0 0 6.91 15.5h.59a2.5 2.5 0 0 0 2.5-2.5V11h4v2a2.5 2.5 0 0 0 2.5 2.5h.59a2.5 2.5 0 0 0 2.24-1.46l2.25-4.5c.39-.73.39-1.62 0-2.35zM9 10H7v2H6v-2H4V9h2V7h1v2h2v1zm7.5 2a1 1 0 1 1 0-2 1 1 0 0 1 0 2zm2-2a1 1 0 1 1 0-2 1 1 0 0 1 0 2z" />
      </svg>
    </div>
  );
}

// Recent Ad Activity Icon
export function RecentAdActivityIcon({ size = 36 }: { size?: number }) {
  return (
    <div
      style={{ width: size, height: size }}
      className="rounded-xl flex items-center justify-center bg-[#00A3FF] text-white shadow-sm shrink-0"
    >
      <svg viewBox="0 0 24 24" width={size * 0.65} height={size * 0.65} fill="none" stroke="currentColor" strokeWidth="2">
...
// Reels Icon (Clapperboard pink/orange gradient)
export function ReelsIcon({ size = 36 }: { size?: number }) {
  return (
    <div
      style={{ width: size, height: size }}
      className="rounded-xl flex items-center justify-center bg-gradient-to-tr from-[#FD1D1D] via-[#F56040] to-[#FFDC80] text-white shadow-sm shrink-0"
    >
      <svg viewBox="0 0 24 24" width={size * 0.65} height={size * 0.65} fill="currentColor">
        <path d="M19.5 3h-15C3.12 3 2 4.12 2 5.5v13C2 19.88 3.12 21 4.5 21h15c1.38 0 2.5-1.12 2.5-2.5v-13C22 4.12 20.88 3 19.5 3zm-.5 15H5V6h14v12zm-8-2l5-4-5-4v8z" />
      </svg>
    </div>
  );
}
